...
export function renderPrizeEmojiGrid(containerId, selectedEmoji, onChange) {
  ...
  const kbBtnId = `${containerId}-kb-btn`;

  let kbInput = document.getElementById(`${containerId}-kb-input`);
  if (!kbInput) {
  ...
  const isCustom = selectedEmoji && PRIZE_EMOJIS.indexOf(selectedEmoji) === -1;
  if (isCustom) { kbInput.value = selectedEmoji; kbInput.style.display = 'block'; }

  function buildGrid(showAll) {
    const visible = showAll ? PRIZE_EMOJIS.slice(0, -1) : PRIZE_EMOJIS.slice(0, SHOW_INITIAL);
    ...
    grid.querySelectorAll('.task-emoji-opt').forEach(el => {
      if (el.id === `${containerId}-show-more`) {
        ...
        el.onclick = () => {
          ...
          kbInput.style.display = 'none'; kbInput.value = '';
          const kb = document.getElementById(kbBtnId);
          if (kb) { kb.style.background = '#F3F0FF'; kb.style.borderStyle = 'dashed'; }
...
export function startPrizeTour(familyId) {
  ...
  function showStep(idx) {
    ...
    setTimeout(() => {
      ...
      const fitsBelow = rect.bottom + 180 < window.innerHeight - NAV_H;
...
export function startManagePrizesTour(familyId) {
  const steps = [
    {
      el: '#prizes-filter',
      title: 'סינון פרסים 🔍',
      text: 'סנן לפי ילד או לפי מחיר בכוכבים — כדי למצוא בקלות את הפרס שחיפשת'
    },
    {
      el: '#prizes-list',
      title: 'ניהול הפרסים ✏️',
      text: 'לחץ על כל פרס כדי לערוך את פרטיו, להסתיר אותו מהילד — או למחוק לגמרי'
    },
  ];
  ...
  const overlay = document.createElement('div');
  overlay.className = 'tour-overlay';
  overlay.id = 'manage-prizes-tour-overlay';
  overlay.addEventListener('touchmove', (e) => e.preventDefault(), { passive: false });
  ...
  function endTour() {
    ...
    setTimeout(() => {
      overlay.remove();
      document.body.style.overflow = '';
      localStorage.setItem(`managePrizesTourDone_${familyId || 'none'}`, '1');
    }, 540);
  }
