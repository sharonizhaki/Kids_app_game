...
// -------- RENDER WEEKLY GRAPH --------
export function renderWeekGraph() {
  const cs       = state.childState;
  const dailyPts = cs?.dailyPts || {};

  const barsEl = document.getElementById('week-graph-bars');
  const daysEl = document.getElementById('week-graph-days');
  ...
  // 7 ימים אחרונים: מ-6 ימים אחורה עד היום
  const days = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date(today);
    d.setDate(today.getDate() - i);
    const key = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    days.push({ key, dayOfWeek: d.getDay(), isToday: i === 0, pts: dailyPts[key] || 0 });
  }

  const maxPts = Math.max(...days.map(d => d.pts), 1);

  barsEl.innerHTML = days.map(d => {
    const h    = Math.max(4, Math.round((d.pts / maxPts) * 52));
    const cls  = d.isToday ? 'today' : d.pts > 0 ? 'has-pts' : '';
    return `
      <div class="week-graph-bar-wrap">
        <div class="week-graph-bar ${cls}" style="height:${h}px;" title="${d.pts} ⭐"></div>
      </div>`;
  }).join('');

  daysEl.innerHTML = days.map(d =>
    `<div class="week-graph-day${d.isToday ? ' today' : ''}">${DAY_SHORT[d.dayOfWeek]}</div>`
  ).join('');
}
