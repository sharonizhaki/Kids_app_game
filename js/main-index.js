  ...
  sh.innerHTML = `<div class="modal-handle"></div>
    ...
    <div class="modal-body">
      ...
      <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:6px;margin-bottom:8px;">
        ${CHILD_EMOJIS.slice(0,-1).map(e => `<div class="emoji-opt${e === obEmoji ? ' selected' : ''}" data-emoji="${e}" style="font-size:1.6rem;aspect-ratio:1;display:flex;align-items:center;justify-content:center;border-radius:12px;cursor:pointer;">${e}</div>`).join('')}
